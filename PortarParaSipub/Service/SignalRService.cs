...
namespace PlataformaOperacional.Service.Middleware
{
    ...
    public class SignalRService : IAsyncDisposable
    {
        ...
        // Progresso de QUALQUER operação (chave, estado) — para componentes globais
        // como a BarraProgresso do layout, que exibem o que estiver rodando sem
        // conhecer a chave. Só dispara para chaves com escuta ativa
        // (IniciarEscutaDaOperacao chamada por alguma página).
        ...
        public Task IniciarEscutaDaOperacao(string hubConnectId)
            => RegistrarEscutaAsync(hubConnectId, conexao =>
                conexao.On<int, ObservadorAutomacao>(hubConnectId, async (progresso, observer) =>
                {
                    ...
                    // 1b. Notifica os ouvintes globais (ex.: BarraProgresso do layout)
                    AoReceberProgressoGlobal?.Invoke(hubConnectId, observer);
